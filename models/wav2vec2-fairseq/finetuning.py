...
def train(rank, world_size, args):
    ...
    if args.dataset == 'physionet':
        ...
        train_path = '../../data/mitdb_rhythm_sampled_csv/'
    else:
        ...
        with tqdm(total = len(train_loader), desc = f"Train({rank}) {epoch}") as pbar:
            for step, data in enumerate(train_loader):
                ...
                with torch.no_grad():
                    ...
                    label.resize_(label_.size()).copy_(label_)
                del val, label_, data
                ...
                with torch.autograd.profiler.record_function("forward"):
                    net_output = model(input, mask = False, features_only = True)
                    print(net_output['x'].shape)
                    exit(0)

                    # TODO
                    # should be modified from this line
                    # Implement finetuning modules (rhythm classifier, ... )
